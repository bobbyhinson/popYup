...
$(document).ready(function() {
	...
	var shift = '0';
	if(settings.fromTop == 50){var shift = '-50%'};
	...
	if ( $('.popYup-open').attr('rel') == 'iframe' || $('.popYup-open').attr('rel') == 'ajax' || $('.popYup-open').attr('rel') == 'image' || $('.popYup-open').attr('rel') == 'inline' ){
		$('body').append('<div pY="yes" class="popYup"></div>');
	};
	...
	$('.popYup-open').click(function() {
		...
		if ( $(this).attr('rel') == 'iframe' ) {
			...
			$('#pyIframe .pyContent').append('<iframe src=" ' + pySrc + ' "></iframe>');
			$('#pyIframe .pyContent iframe').load(function() { 
				$('#pyIframe').show();
				killLoader();
			});
			...
			$('#pyImage .pyContent').html('<img src="' + pySrc + '" />');		
			$('#pyImage .pyContent img').load(function() { 
				$('#pyImage').show();
				killLoader();
			});
			...
			$('#pyInline .pyContent').html(inlineSrc);		
			$('#pyInline').show(function(){killLoader();});
		}else if ( $(this).attr('rel') == 'local' ) {
			$(pySrc).show(function(){killLoader();}); 
		};
		$('body').addClass('lock');
		$('body').append('<div id="pyOverlay"><div id="pyLoader">'+settings.loader+'</div></div>');
		return false;			
	});
	$(document).on('click','.popYup-close',function() {closePY();return false;});
	if(settings.overlayClose == true){
		$(document).on('click','#pyOverlay',function() {closePY();});
	};
});
function killLoader(){$('#pyLoader').remove()};
function closePY() {
	$('.popYup').hide(); 
